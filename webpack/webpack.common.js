exports.getCommon = function (commonOptions = {}) {
  ...
  const jsApiUrl = process.env.ARCGIS_JSAPI_URL || 'https://js.arcgis.com/4.29/';
  ...
  function createStatsWriterPlugin(chunkName){
    ...
    return new StatsWriterPlugin({
      ...
      transform: (data, options) => {
        ...
        d.chunks = data.chunks.filter(c => c.files.length && c.files[0].startsWith('widgets/chunks')).map(chunck => {
          const modules = []
  ...
  exports.externalFunction = function({context, request}, callback) {
    if (isRequestExternal(request)) {
      ...
      } else if (['analysis-components', 'analysis-core', 'analysis-shared-utils', 'analysis-raster-function-utils', 'analysis-tool-app'].includes(request)) {
  ...
  function isRequestExternal(request){
    ...
    const fullMatchPackages = [
      ...
      'analysis-components', 'analysis-core', 'analysis-shared-utils', 'analysis-raster-function-utils', 'analysis-tool-app'
